...
///
/// Also the floor applied to a *stored* w20 on read: a persisted value below
/// it is a degenerate fit from an older build, not a personalization.
...
impl FSRSOptimizer {
    ...
    /// straight into its lower bound. Not hypothetical: an agent memory
    /// store's access log is success-dominated by construction. Upstream
    /// measured a 2,929-memory store running at w20 = 0.0104 for a month;
    /// Eggs' 860-memory store was measured at 0.0104 on 2026-09-03, with
    /// every node compressed into 0.62..0.90 retention and February memories
    /// untouched for seven months still at 0.63. Success-only data is
    /// insufficient evidence, not evidence of immortal memory.
...
mod tests {
    ...
    fn test_optimize_decay() {
        ...
        assert!(optimized_decay >= MIN_DECAY_BOUND);
    ...
    /// REGRESSION (v2.2.10, upstream v2.6.0): a success-only history must
    /// not produce a fit. The measured failure: a real store's access log fed
    /// the optimizer success-dominated data, the fit collapsed into the old
    /// 0.01 lower bound, and store-wide decay silently stopped.
    #[test]
    ...
    /// Fewer than MIN_FORGETTING_EVIDENCE failures is still "no evidence".
    #[test]
    fn a_handful_of_failures_is_not_evidence() {
        let mut optimizer = FSRSOptimizer::new();
        let now = Utc::now();
        optimizer.add_reviews((0..200).map(|i| ReviewLog {
            timestamp: now - Duration::days(i as i64),
            rating: if i < MIN_FORGETTING_EVIDENCE - 1 { 1 } else { 3 },
            stability: 5.0,
            difficulty: 5.0,
            elapsed_days: 1.0 + (i as f64 * 0.5),
        }));
        assert!(!optimizer.has_forgetting_evidence());
        assert_eq!(optimizer.optimize_decay(), FSRS6_WEIGHTS[20]);
    }
