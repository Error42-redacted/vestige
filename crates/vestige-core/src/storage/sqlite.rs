...
impl SqliteMemoryStore {
    ...
    /// (Ported from upstream v2.2.1, issue #103.)
    ...
    pub fn run_consolidation(&self) -> Result<ConsolidationResult> {
        ...
        //
        // OPT-OUT (backfill-safety, ported from upstream v2.2.1 #103): auto-fire
        // is ON by default — it shipped and was documented in v2.2.0, so we keep
        // the behavior — but is now bounded and disableable. It mutates FSRS
        // scores on the canonical store and can lift a memory across a downstream
        // consolidation floor, so a consumer that reads `stability` as a
        // durability gate can turn it off with VESTIGE_BACKFILL_AUTOFIRE=0 (or
        // false/off/no). The `backfill` MCP tool + CLI remain available for
        // on-demand, operator-driven backfill regardless of the gate. The promote
        // is bounded: both the auto-fire and manual paths call
        // promote_memory_backfill (stability = MIN(stability*1.5, stability+365))
        // so repeated per-(cause, failure) promotions cannot inflate without
        // bound (the prior comment claimed promote_memory was capped — it was not).
