...
| `VESTIGE_STATE_TTL_DAYS` | `30` | v2.2.10: default lifetime of a `state` memory (`smart_ingest` `node_type: "state"` — version numbers, deploy status, progress). After it expires the memory is down-ranked to the bottom of recall (x0.1) and reported `currentlyValid: false`, still retrievable for audit. `0` disables the default expiry |
| `VESTIGE_RETENTION_TARGET` | `0.8` | Report-only since v2.2.10: the consolidation cycle logs how many memories sit below 0.3 retention and records the average in `retention_snapshots`; it **never deletes**. The explicit `maintain {action:"gc"}` command (dry-run by default, protected memories exempt) is the only collector. `vestige health` also reports whether average retention meets this target |
| `VESTIGE_SUPERSEDE_MODE` | `suggest` | v2.2.5: consent mode for smart_ingest's gate-driven supersession. `suggest` (default) creates + returns `supersedeCandidate` for explicit confirm; `auto` restores guarded auto-supersede (demote + bitemporal stamp, fully reversible via dedup undo — v2.2.6 restores the exact pre-supersede FSRS state too); `off` never supersedes from the gate. Explicit `supersede='<id>'` is honored in every mode |
